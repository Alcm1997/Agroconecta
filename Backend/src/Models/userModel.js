...
exports.getUserByUsername = async (username) => {
  const result = await pool.query('SELECT * FROM usuario WHERE UPPER(username) = UPPER($1)', [username]);
  return result.rows[0];
};

// Buscar usuario por email (insensible a mayúsculas)
exports.getUserByEmail = async (email) => {
  const result = await pool.query('SELECT * FROM usuario WHERE UPPER(email) = UPPER($1)', [email]);
  return result.rows[0];
};
