...
// Obtener unidad por descripción (para validaciones)
exports.getByDescripcion = async (descripcion) => {
  const { rows } = await pool.query('SELECT * FROM unidad_medida WHERE UPPER(descripcion) = UPPER($1)', [descripcion]);
  return rows[0];
};
